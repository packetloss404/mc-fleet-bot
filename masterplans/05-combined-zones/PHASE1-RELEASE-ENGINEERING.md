...
This ordering follows Masterplan 05: prove the bounded design first, build deep shells before surface loads, form the mountain before dependent surface work, fit out only accepted shells, and commission transport last. Every physical node is a separate transaction against a new immutable source snapshot. No release inherits a stale preflight from an earlier node.

## What must be authored before the first block

1. Close the three remaining masterplan decisions with machine-readable evidence. D01 selects same-world placement, D03 deletes unsurveyed L2, D04 selects a fully clear-spanned empty rail reservation, and D07 now binds fact-checked architectural-composite wording while omitting C2. D02's exact civil coordination still lacks geotechnical/structural/outfall/C01 acceptance, D05's full-height baseline still lacks reviewed buffers and future hydrology ownership, and D06's internal freeze still lacks external life-safety continuations and commissioning. Exact per-owner construction cells remain a separate G03 requirement.
...
4. Retain the three hash-bound zero-margin default-deny relic cores and freeze reviewed positive buffers for both igloos and the shipwreck. Resolve the east-igloo record whose 280-cell core is entirely air in the copied snapshot, and prove complete construction-volume and poststate preservation. The core census alone is insufficient for construction.
...
The lifecycle is deliberately non-cyclic: G01–G14 must pass before a physical transaction can start, G15 controls execution, and the release cannot become accepted until G01–G19 all pass. Missing, stale, partial, hash-drifting or narratively equivalent evidence fails closed. The live entity report must bind all forward operation hashes and be no more than five minutes old at transaction start. Any failed transaction must finish the journal-proven rollback, freeze the world, capture a new immutable snapshot and restart the complete gate sequence.
...
The current validator is also fail-closed against placeholder advancement evidence. A resolved contract decision must match a `RESOLVED` record inside its bound decision ledger; relic cores must match the bound census; a reviewed buffer must be an approved, hash-bound exact set; the bound decision, relic, C1, and site artifacts must each report their semantic PASS; and G01–G14 must have explicit PASS evaluations bound to authority evidence. The present contract intentionally has no current gate evaluations and keeps all release nodes `BLOCKED`. Advancing a node requires the missing project release tooling and a reviewed contract-state transition; the current-state validator cannot be treated as an execution wrapper.
