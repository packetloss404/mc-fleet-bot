...
const guardCells = contract.selectedGeometry?.handrailAndGuard?.cells?.map((entry) => ({ cell: p(entry), targetState: entry.targetState, role: entry.role })) ?? [];
fail(guardCells.length === 100 && guardCells.every((r) => r.targetState === 'minecraft:polished_deepslate_wall'), 'exact 100-cell PDS guard/handrail contract required');
