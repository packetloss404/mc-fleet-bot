#!/usr/bin/env bash
#
# claude-tmux.sh - attach to (or create) a persistent tmux session running
# Claude Code, so SSH drops never kill your session.
#
# Run this ONE command after you SSH in:
#     /opt/stacks/mc-fleet-bot/scripts/claude-tmux.sh
#
# - First run:  creates a tmux session named "claude" in the project dir and
...
WORKDIR="${CLAUDE_TMUX_WORKDIR:-/opt/stacks/mc-fleet-bot}"
CLAUDE_CMD="${CLAUDE_TMUX_CMD:-claude}"
...
if ! command -v "${CLAUDE_CMD%% *}" >/dev/null 2>&1; then
  echo "WARNING: '${CLAUDE_CMD%% *}' not found on PATH." >&2
