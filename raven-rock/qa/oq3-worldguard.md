...
`raven-rock/planning/coordinates.yaml` `meta.site_conditions` asserted "stock Paper with **NO plugins** —
...
## 3. What `mainstreet-america/integration/worldguard.yaml` specified vs. what was applied
...
`raven-rock/integration/worldguard.yaml` needed **no** bound changes: its y61 cap (one below MSA's y62
...
as-built bounds in `ravensreach/qa/audit-2026-07-25.md` (graded site x[−132,−38] z[−425,−333], grove out
...
`raven-rock/`: `planning/coordinates.yaml`, `planning/site-plan.md` (§1 + Assumptions),
...
`mainstreet-america/`: `planning/coordinates.yaml`, `planning/site-plan.md` (§1 + Assumptions),
