...
* "Comes with a test if a meaningful regression test is feasible":
  - #1 — **DONE 2026-08-08:** `test/voyager/VoyagerLoop.findGroundedBuildOrigin.test.ts`
    ...
    end-to-end through the real Blueprint.ts pipeline, and the
    happy-path goal production. The first draft of this test mocked the
    blueprint pipeline and asserted an aspirational `activeLongTermGoal`
    state that the code never actually persisted; both the test and
    the inline comment in `decomposeAndSetLongTermGoal` were updated
    in a follow-up pass to match the real behavior (goal is local-only,
    not promoted to `activeLongTermGoal` on the failure path).
