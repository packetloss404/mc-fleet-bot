...
1. `docs/raven-rock/planning/site-plan.md` — **declared master**; §1.1 vertical stacking, §2 zones, §3 circulation.
2. `docs/raven-rock/planning/coordinates.yaml` — coordinate manifest, `consistency_check`, `assumptions_and_gaps`.
3. `docs/raven-rock/planning/buildings.yaml` — building records RR-B1…RR-B4.
4. `docs/raven-rock/planning/palettes.yaml` — Z-* palettes (drove the drawing colour language: greenstone
   green, institutional grey concrete, copper/iron accents, cyan water, yellow/black hazard).
5. `docs/raven-rock/planning/open-questions.md` — **the DECIDED table at the top (six items signed off 2026-07-24)**.
6. `docs/raven-rock/references/manifest.yaml` — REF-001…REF-018, confidence vocabulary, conflicts.
7. `docs/raven-rock/qa/qa-report.md`, `docs/raven-rock/integration/*.yaml` — read for cross-checking only (see conflicts).
8. `docs/mainstreet-america/qa/as-built-survey.md` — MSA foundation y62 / road spine y63 / surface y64 /
   Guest Center solid y63→y79 at x −70…+70, z ≈ 90…180.
9. `docs/mainstreet-america/planning/buildings.yaml`, `site-plan.md` — MSA building positions used for the
...
| `docs/mainstreet-america/planning/open-questions.md` | ❌ stale — still asks MSA to reserve x 113–127, z 53–67 |
...
| **MSA building positions on section A–A** | Projected from `docs/mainstreet-america/planning/buildings.yaml` onto the cut plane and labelled *schematic — true z varies*. Their true z spans −235…+200; the section cut is at z ≈ −15. |
...
Circulation: **T1** N4 (0,y18,−285) → N1 (0,−6,−120) → Cavern A (0,−12,−45) · **T2** N3 (−150,y18,+285)
→ turn (−150,y2,+190) → Cavern B west end (−45,−10,+130) · **T3** N5 (285,y18,−30) → N2 (180,y0,−30) → Cavern A (75,−12,−15) ·
...
`docs/mainstreet-america/planning/open-questions.md`, delete the DOCUMENT CONFLICT footer from both sheets
