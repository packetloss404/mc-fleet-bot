...
12. **Validate `mining.minDigY` in `SECTION_SPECS`** (`config.ts:457-464`). Cost: one spec entry. Value: high relative to cost — a hand-edited type slip currently fail-opens the dig floor that prevents fleet entombment.
